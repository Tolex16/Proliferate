...
public class OAuthLoginServiceImpl implements OAuthLoginService {
    ...
    if (email == null) {
        throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid Google token: email could not be retrieved.");
    }
    ...
    public LoginResponse loginWithFacebook(Oauth2Request request) throws IOException, ParseException {
        // 1. Verify Facebook token and fetch email
        String email = verifyFacebookTokenAndFetchEmail(request.getIdToken());

        if (email == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid Facebook token or no email found");
        }
  ...
  private String verifyGoogleToken(String idTokenString) throws GeneralSecurityException, IOException {
    ...
    // Step 1: Verify Facebook Token and Fetch Email
    public String verifyFacebookTokenAndFetchEmail(String accessToken) throws IOException, ParseException {
        // Verify token and check expiry
        validateFacebookToken(accessToken);

        // Fetch email after validating token
        return fetchFacebookUserEmail(accessToken);
    }

    // Step 2: Validate Facebook Token (including expiry)
    private void validateFacebookToken(String accessToken) throws IOException, ParseException {
        ...
        try (CloseableHttpResponse response = httpClient.execute(request)) {
            ...
            if (statusCode != HttpStatus.OK.value()) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid Facebook access token, status code: " + statusCode);
            }

            String jsonResponse = EntityUtils.toString(response.getEntity());
            Map<String, Object> result = objectMapper.readValue(jsonResponse, Map.class);

            // Ensure that the "data" field is present in the response
            if (!result.containsKey("data")) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Invalid response from Facebook API, 'data' field is missing");
            }
            ...
            if (isValid == null || !isValid) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Facebook token is invalid or expired");
            }
            ...
            if (data.containsKey("expires_at")) {
                ...
                if (expiresAt < currentTime) {
                    throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Facebook token has expired");
                }
            }
        }
    }

    // Step 3: Retrieve User Profile Information (including email)
    private String fetchFacebookUserEmail(String accessToken) throws IOException, ParseException {
        ...
        try (CloseableHttpResponse response = httpClient.execute(request)) {
            ...
            if (statusCode != HttpStatus.OK.value()) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Failed to fetch user email from Facebook, status code: " + statusCode);
            }
            ...
            if (!result.containsKey("email")) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "No email found in Facebook response");
            }
